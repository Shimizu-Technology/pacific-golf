module Api
  module V1
    class EmployeeNumbersController < BaseController
      skip_before_action :authenticate_user!, only: [:validate]
      before_action :set_tournament_for_admin!, except: [:validate]
      ...
      def update
        employee_number = @tournament.employee_numbers.find(params[:id])
      ...
      def destroy
        ...
        if employee_number.used?
          return render json: { error: "Cannot delete an employee number that has been used." }, status: :unprocessable_entity
        end
      ...
      def release
        employee_number = @tournament.employee_numbers.find(params[:id])
        return render json: { error: "This employee number is not in use" }, status: :unprocessable_entity unless employee_number.used?
      ...
      def set_tournament_for_admin!
        tournament_id = params[:tournament_id]
        return render json: { error: "Tournament not found or not specified" }, status: :not_found if tournament_id.blank?
